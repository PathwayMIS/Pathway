...
if ( ! function_exists ( 'quote_calculator_create_table' ) ) {
	function quote_calculator_create_table(){
		...
		$sql = 'CREATE TABLE IF NOT EXISTS `' . $wpdb->prefix . 'quote_calculator_colour` (
			...
			`colour_price` decimal(20,2) NOT NULL DEFAULT "0.00",
		...
		if( isset( $_GET['quote_calculator_create_pdf'] ) && is_numeric( $_GET['quote_calculator_create_pdf'] ) ){
			...
			if( ! empty( $estimate ) ){
				...
				if ( ! empty( $estimate['estimates_delivery'] ) ) {
					$delivery_price = $wpdb->get_results( 'SELECT `delivery_price` FROM `' . $wpdb->base_prefix . '11_quote_calculator_delivery` WHERE delivery_title="' . $estimate['estimates_delivery'] . '"', ARRAY_A );
					$delivery_price = $delivery_price[0];
					$delivery_price = number_format( $delivery_price["delivery_price"], 2 );
					$vat = $wpdb->get_results( 'SELECT `estimate_section_vat` FROM `' . $wpdb->base_prefix . '11_quote_calculator_estimate_sections` WHERE estimate_sections_estimate_id="' . $estimate['estimates_id'] . '"', ARRAY_A );
				...
				class PDF extends FPDF{
					...
					function Header()	{
						...
						if( false === $flag_header ){
							...
							// Logo
							$this->Image( plugin_dir_path( __FILE__ ) . 'images/logo.png', 10, 10, 45 );
                            $this->Image( plugin_dir_path( __FILE__ ) . 'images/google_rate.jpg', 145, 10, 45 );
                            $this->Ln(30);
							$this->SetFont( 'Arial', '', 20 );
							$this->SetTextColor( 158, 51, 147 );
							if( 'estimate' == $status ) {
					...
					function Footer(){
						...
						if( true === $flag_footer ) {
                            ...
                            $this->Image( plugin_dir_path( __FILE__ ) . 'images/logo_tonbridge.png', 122, 270, 20 );
							$this->SetY(-30);
							$this->SetFont( 'Arial', '', 10 );
							$this->SetTextColor( 158, 51, 147 );
