...
class WP_FullCalendar{
	...
	public static function localize_script(){
		...
		if( ! empty( $qc_dashboard_options ) && ! empty( $qc_dashboard_options['calendar_view_options']['day_view'] ) ){
			$week_days = array( 'sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday' );
			foreach( $week_days as $key => $value ){
				if( ! in_array( $value, $qc_dashboard_options['calendar_view_options']['day_view'] ) ){
					$js_vars['hiddenDays'][] = $key;
				}
			}
		}
	...
	public static function ajax(){
	    ...
	    $item_date_counts = array();
			...
			/* !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			Change selection from post_date to deadline */
			$args['meta_query'] = array(
				array(
					'key'     => 'deadline',
					'value'   => str_replace( '-', '', $_REQUEST['start'] ),
					'compare' => '>=',
				),
				array(
					'key'			=> 'deadline',
					'value'   => str_replace( '-', '', $_REQUEST['end']),
					'compare' => '<',
				),
			);
	    ...
	    while ( $the_query->have_posts() ) { $the_query->the_post();
				...
				if( ! in_array( $project_stage, array( 'Print Production', 'Jobs Out', 'Delivery' ) ) ) {
					continue;
				}
    		$color = "#a8d144";
				/* !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	    	...
	    	$post_timestamp = strtotime($post->post_date);*/
				$post_date = get_post_meta( get_the_ID(), 'deadline', true );
				$post_timestamp = strtotime($post_date);
				/* !!!!!!!!!!!!!!!!!!!!!!!!!!!!!
				End chage */
	    	if( empty($item_date_counts[$post_date]) || $item_date_counts[$post_date] < $limit ){
	    		/* !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
					Change post_title to client and reference */
					//$title = $post->post_title;
					$title = get_post_meta( get_the_ID(), 'client', true ) . ' - ' . get_post_meta( get_the_ID(), 'reference', true );
					$current_blog_id = get_current_blog_id();

					$qc_dashboard_options = get_option( 'qc_dashboard_options' );

					if( ! empty( $qc_dashboard_options['calendar_color_options'] ) ){
						if( 'Print Production' == $project_stage ) {
							$color = $qc_dashboard_options['calendar_color_options']['print_production'] . ' !important';
						} elseif( 'Jobs Out' == $project_stage ){
							$color = $qc_dashboard_options['calendar_color_options']['jobs_out'] . ' !important';
						} elseif( 'Delivery' == $project_stage ){
							$color = $qc_dashboard_options['calendar_color_options']['deliver'] . ' !important';
						}
					} else {
						if( 'Print Production' == $project_stage ) {
			...
			//$the_query = new WP_Query( $args );
			//error_log(print_r($the_query, true) . PHP_EOL, 3, dirname(__FILE__) . '/error.log' );
			//$all_postmeta = get_post_custom(1280);
			//error_log(print_r($all_postmeta, true) . PHP_EOL, 3, dirname(__FILE__) . '/error.log' );
