...
class Clientside_Setup {
	...
	static function action_enqueue_admin_styles() {
		...
		// Always-use CSS
		wp_enqueue_style( 'clientside-admin-css', plugins_url( 'css/clientside-admin-1.3.8.css', __FILE__ ), array(), null );
		...
		if ( Clientside_Options::get_saved_option( 'enable-admin-theme' ) ) {
			wp_enqueue_style( 'clientside-theme-css', plugins_url( 'css/clientside-admin-theme-1.3.8.min.css', __FILE__ ), array( 'clientside-admin-css', 'thickbox' ), null );

			// Additional external plugin support
			if ( Clientside_Options::get_saved_option( 'enable-plugin-support' ) ) {
				wp_enqueue_style( 'clientside-plugin-support-css', plugins_url( 'css/clientside-plugin-support-1.3.8.min.css', __FILE__ ), array( 'clientside-theme-css' ), null );
			}
	...
	static function action_enqueue_admin_scripts() {
		...
		// Clientside theme JS
		wp_enqueue_script( 'clientside-admin-js', plugins_url( 'js/clientside-admin.js', __FILE__ ), $dependencies, '1.3.8' );
	...
	static function action_enqueue_login_styles() {
		...
		if ( Clientside_Options::get_saved_option( 'enable-login-theme' ) ) {
			wp_enqueue_style( 'clientside-login-css', plugins_url( 'css/clientside-login-1.3.8.min.css', __FILE__ ), array(), null );
		}
	...
	static function action_enqueue_login_scripts() {
		...
		// Only if login page theming is enabled
		if ( Clientside_Options::get_saved_option( 'enable-login-theme' ) ) {
			wp_enqueue_script( 'clientside-login-js', plugins_url( 'js/clientside-login.js', __FILE__ ), array( 'jquery' ), '1.3.8' );
		}
	...
	static function action_enqueue_editor_styles() {
		...
		// Load the stylesheet
		add_editor_style( plugins_url( 'css/clientside-editor-1.3.8.min.css', __FILE__ ) );
