...
if ( ! function_exists( 'quote_calculator_frontend_head' ) ) {
	function quote_calculator_frontend_head() {
		...
		if( ! empty( $current_page ) && in_array( $current_page, array( 'dashboard', 'estimate' ) ) ){
			...
			wp_enqueue_script( 'quote_calculator_script', plugins_url( 'js/script.js', __FILE__ ), array( 'jquery-ui-core', 'jquery-ui-widget', 'jquery-ui-autocomplete', 'jquery-ui-button', 'jquery-ui-tooltip', 'jquery-ui-datepicker' ) );
		...
		if( isset( $_GET['quote_calculator_create_pdf'] ) && is_numeric( $_GET['quote_calculator_create_pdf'] ) ){
			...
			if( ! empty( $estimate ) ){
				...
				class PDF extends FPDF{
					...
					function Header()	{
						...
						if( false === $flag_header ){
							...
							// Logo
							$this->Image( plugin_dir_path( __FILE__ ) . 'images/logo.jpg', 10, 10, 45 );
							...
							$this->Cell( 135 );
							$this->Cell( 50, 5, 'www.absolutecp.co.uk', 0, 1 );
							$this->Ln(5);
							$flag_header = true;
						}
					}
					...
					function Footer(){
						...
						if( true === $flag_footer ) {
							...
							$this->Cell( 0, 5, $qc_dashboard_options['company_details']['company_address_line1'] . ', ' . $qc_dashboard_options['company_details']['company_address_line2'] . ', ' . $qc_dashboard_options['company_details']['company_address_city'] . ', ' . $qc_dashboard_options['company_details']['company_address_location'] . ', ' . $qc_dashboard_options['company_details']['company_address_postal_code'], 0, 1, 'C' );
							$this->Cell( 0, 5, $qc_dashboard_options['company_details']['phone'] . '         ' . $qc_dashboard_options['company_details']['email'] . '          www.absolutecp.co.uk', 0, 1, 'C' );
							$this->SetFont( 'Arial', '', 7 );
							$this->Cell( 0, 5, 'Registered Office: '. $qc_dashboard_options['company_details']['registered_address_line1'] . ', ' . ( ! empty( $qc_dashboard_options['company_details']['registered_address_line2'] ) ? $qc_dashboard_options['company_details']['registered_address_line2'] . ', ' : '' ) .  $qc_dashboard_options['company_details']['registered_address_city'] . ', ' . $qc_dashboard_options['company_details']['registered_address_location'] . ', ' . $qc_dashboard_options['company_details']['registered_address_postal_code'] . '     Company Registration Number ' . $qc_dashboard_options['company_details']['registration_number'] . '     Registered in England and Wales.   VAT No: ' . $qc_dashboard_options['company_details']['vat_number'], 0, 1, 'C' );
						} else {
							$this->SetY(-15);
				...
				foreach( $estimate_sections as $estimate_section ) {
					...
					foreach( $estimate_section['estimate_section_qty'] as $key => $value ){
						...
						if ( 'delivery_note' != $status ) {	
							...
							$pdf->Cell( 60);
							$pdf->Write( 6, urldecode( '%A3' ) . number_format( $estimate_section['estimate_section_cost_without_vat'][ $key ], 2 ) . ' ' );
				...
				if( ! isset( $_GET['without_total'] ) && 'delivery_note' != $status){
					...
					$pdf->Cell( 100 );
					$pdf->SetFont( 'Arial', '', 10 );
					$pdf->Cell( 14, 6, urldecode( '%A3' ) . number_format( $estimate['estimates_cost'], 2 ), 0, 0 );
					$pdf->SetFont( 'Arial', '', 8 );
					if( 'estimate' == $status ) {
				...
				$pdf->SetFont( 'Arial', '', 10 );
				$pdf->MultiCell( 155, 6, $estimate['estimates_notes'], 0, 1 );
				$pdf->SetFont( 'Arial', 'B', 10 );
