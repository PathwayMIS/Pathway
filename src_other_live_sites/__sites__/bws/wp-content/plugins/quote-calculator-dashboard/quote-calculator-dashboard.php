...
class QS_Dashboard {
	...
	function create_dashboard() {
		...
		$current_month_sales			= $wpdb->get_var( 'SELECT COUNT(*) FROM `' . $wpdb->prefix . 'quote_calculator_estimates` WHERE `estimates_status` = "sales"' );
		$current_month_design			= $wpdb->get_var( 'SELECT COUNT(`estimate_section_id`) FROM `' . $wpdb->prefix . 'quote_calculator_estimate_sections`, `' . $wpdb->prefix . 'quote_calculator_estimates` WHERE `estimate_sections_estimate_id` = `estimates_id` AND `estimate_section_category` = 3' );
		$current_month_completed	= $wpdb->get_var( 'SELECT COUNT(*) FROM `' . $wpdb->prefix . 'quote_calculator_estimates` WHERE `estimates_date` >= ' . strtotime( date( 'Y-m-1' ) ) . ' AND `estimates_status` = "completed"' );
