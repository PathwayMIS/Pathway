		...
		if( isset( $_GET['quote_calculator_create_pdf'] ) && is_numeric( $_GET['quote_calculator_create_pdf'] ) ){
			...
			if( ! empty( $estimate ) ){
				...
				if ( ! empty( $estimate['estimates_delivery'] ) ) {
					...
					$vat = $wpdb->get_results( 'SELECT `estimate_section_vat` FROM `' . $wpdb->base_prefix . 'quote_calculator_estimate_sections` WHERE estimate_section_id="' . $estimate['estimates_id'] . '"', ARRAY_A );
