...
( function( $ ){
	$( document ).ready( function(){
		...
		$( document ).on( 'click', '.add-qty', function(){
			...
			newQtyBlock.find( 'input' ).attr( 'data-qty', newQty );
			newQtyBlock.find( 'input' ).val( 0 );
		...
		$( document ).on( 'click', '.digital-printing .quote-calculator-remove', function(){
			if( false !== $( this ).hasClass( 'remove-qty' ) ){
				...
				parent.find( '.quote-calculator-copy.display-cost[data-qty="' + $( this ).prev().find( 'input' ).attr( 'data-qty' ) + '"]' ).remove();
				if( parent.find( '.quote-calculator-copy.display-cost' ).length == 1 ){
					parent.find( '.quote-calculator-copy.display-cost span:first-child:not(.cost_without_vat)' ).addClass( 'hidden' );
				}
			}
		...
		$( 'section ' ).on( 'dblclick', '.cost_without_vat', function(){
			...
			$( '#cost_without_vat' ).on( 'blur', function(calculationDpTotal){
...
function calculationDpTotal( currentElement ){
	( function($){
		var Q1 = 0, Q2 = 0, Q3 = 0, Q4 = 0, Q5 = 0, Q6 = 0, totalFinishing = 0;
		var dataQty;
		...
		if ( undefined == page_count || 0 == parseInt( page_count ) ) {
			page_count = 1;
		}
		var quantity;
		var countQtyBlock	= 0;
		...
		countQtyBlock = parent.find( 'input[name*="estimate_section_qty"]' ).length;

		parent.find( 'input[name*="estimate_section_qty"]' ).each( function( key, select ){
			Q1 = 0, Q2 = 0, Q3 = 0, Q4 = 0, Q5 = 0, Q6 = 0, totalFinishing = 0, dataQty = '';
			quantity = $( this ).val();
			if( 'undefined' == typeof( $( this ).attr( 'data-qty' ) ) ){
				$( this ).attr( 'data-qty', key );
			}
			dataQty	 = $( this ).attr( 'data-qty' );

			/*if( quantity != '' && paper_size != '' ) {
				Q1 = parseInt( quantity ) / parseFloat( paper_size );
			}
			if( isNaN( Q1 ) ){
				Q1 = 0.00;
			}
			...
			if ( impressions != '' ) {
				Q3 = Q2 * parseInt( impressions );
			}
			if( isNaN( Q3 ) ){
				Q3 = 0.00;
			}

			Q4 = Q3 * parseInt( page_count );
			if( isNaN( Q4 ) ){
				Q4 = 0.00;
			}
			...
			parent.find( '.quote-calculator-copy.paper' ).each( function(){
				...
				if( paper != '' && false === isNaN( paper ) ){
					totalPaper += parseFloat( paper );
				}
			});
			if ( 0 == totalPaper ) {
				totalPaper = 1;
			}
			Q5 = parseFloat( totalPaper ) * parseInt( page_count ) * Q1;
			if( isNaN( Q5 ) ){
				Q5 = 0.00;
			}
			...
			parent.find( '.quote-calculator-copy.finishing select[name*="estimate_section_finishing"]' ).each( function(){
				...
				if( finishing != '' && false === isNaN( finishing ) ){
					totalFinishing += parseFloat( finishing );
				}
			});

			Q6 = Q1 * parseInt( page_count ) * parseFloat( totalFinishing );
			if( isNaN( Q6 ) ){
				Q6 = 0.00;
			}
			...
			if( countQtyBlock > 1 ){
				currentDisplayBlock.find( 'span:first-child:not(.cost_without_vat)' ).text( quantity + ' - ' ).removeClass( 'hidden' );
			}
			if( parseFloat( currentDisplayBlock.find( 'input[name*="estimate_section_cost"]' ).val() ) == 0 || quantity == 0 ) {
				currentDisplayBlock.find( '.cost .cost_without_vat' ).html( '£0' );
				currentDisplayBlock.find( '.cost .cost_with_vat' ).html( '( £0 incl VAT )' );
				currentDisplayBlock.find( 'input[name*="estimate_section_cost"]' ).val( 0 );
				currentDisplayBlock.find( 'input[name*="estimate_section_cost_without_vat"]' ).val( 0 );
			}
