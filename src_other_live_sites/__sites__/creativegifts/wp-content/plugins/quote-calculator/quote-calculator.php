...
if ( ! function_exists( 'quote_calculator_get_estimate_item_parent' ) ) {
	function quote_calculator_get_estimate_item_parent( $item_category ){
		switch( $item_category ){
			case 0:
				$parent_id = 69; // Delivery
				break;
			case 1:
				$parent_id = 141; // Digital Printing
				break;
			case 2:
				$parent_id = 142; // Jobs Out
				break;
			case 3:
				$parent_id = 70; // Design
				break;
			case 4:
				$parent_id = 143; // Wide format
				break;
		}
		return $parent_id;
	}
				...
				foreach( $quote_calculator_estimate_sections as $section ){
					...
					if( ! empty( $quantities ) && is_array( $quantities ) ){
						foreach( $quantities as $key => $value ){
							...
							$quote_calculator_estimate_all_sections_tax_code[] = '3';
						}
					} else {
						$quote_calculator_estimate_section['estimate_section_single_cost'] = money_format('%.2n', $quote_calculator_estimate_section['estimate_section_cost_without_vat'] );
				...
				class PDF extends FPDF{
					...
					function Header()	{
						...
						if( false === $flag_header ){
                            ...
                            $this->Image( plugin_dir_path( __FILE__ ) . 'images/pathwaymis_logo.png', 10, 10, 45 );
                            $this->Image( plugin_dir_path( __FILE__ ) . 'images/google_rate.png', 145, 10, 45 );
                            $this->Ln(30);
							...
							if( 'estimate' == $status ) {
								...
								$this->Cell( 58, 10, 'DELIVERY NOTE', 0, 1, 'C' );
								$this->Cell( 77 );
							}
							/*$this->SetFont('Arial', '', 10);
							$this->SetTextColor( 0, 0, 0 );
					...
					function Footer(){
						...
						$b = $rgb[2];*/
						if( true === $flag_footer ) {
                            $this->Image( plugin_dir_path( __FILE__ ) . 'images/logo_maidstone.png', 138, 270, 18 );
                            $this->Image( plugin_dir_path( __FILE__ ) . 'images/logo_tonbridge.png', 161, 270.5, 18 );
                            $this->Image( plugin_dir_path( __FILE__ ) . 'images/offpaper_logo.png', 182, 272, 18 );
                            $this->SetY(-35);
                            $this->SetFont( 'Arial', 'B', 11 );
                            $this->SetTextColor( 0, 9, 67 );
                            $this->Cell( 0, 10, strtoupper($qc_dashboard_options['company_details']['company_name']), 0, 1, 'L' );
                            $this->SetFont( 'Arial', 'B', 8 );
                            $this->SetTextColor( 128, 128, 128 );
                            $this->Cell( 20, 5, 'MAIDSTONE: ', 3, 0, 'L' );
                            $this->SetFont( 'Arial', '', 8 );
                            $this->Cell( 79, 5, $qc_dashboard_options['company_details']['company_address_line1'] . ', ' . $qc_dashboard_options['company_details']['company_address_line2'] . $qc_dashboard_options['company_details']['company_address_line3'] . ', ' . $qc_dashboard_options['company_details']['company_address_city'] . ', ' . $qc_dashboard_options['company_details']['company_address_location'] . ', ' . $qc_dashboard_options['company_details']['company_address_postal_code'], 0, 0, 'L' );
                            $this->SetTextColor( 0, 9, 67 );
                            $this->Cell( 5, 5, urldecode( '%95' ), 0, 0, 'L'  );
                            $this->SetTextColor( 128, 128, 128 );
                            $this->Cell( 20, 5, $qc_dashboard_options['company_details']['phone'], 0, 1, 'L'  );
                            $this->SetFont( 'Arial', 'B', 8 );
                            $this->Cell( 20, 5, 'TONBRIDGE:   ', 3, 0, 'L' );
                            $this->SetFont( 'Arial', '', 8 );
                            $this->Cell( 59, 5, '161 High Street, Tonbridge, Kent, TN9 1BX', 3, 0, 'L' );
                            $this->SetTextColor( 0, 9, 67 );
                            $this->Cell( 5, 5, urldecode( '%95' ), 0, 0, 'L'   );
                            $this->SetTextColor( 128, 128, 128 );
                            $this->Cell( 20, 5, '01732 357 872', 3, 1, 'L' );
                            $this->Cell( 40, 5, $qc_dashboard_options['company_details']['email'], 0, 0, 'L'  );
                            $this->SetTextColor(  0, 9, 67 );
                            $this->Cell( 5, 5, urldecode( '%95' ), 0, 0, 'L'   );
                            $this->SetTextColor( 128, 128, 128 );
                            $this->Cell( 23, 5, $qc_dashboard_options['company_details']['company_url'], 0, 0, 'L' );
                            $this->Cell( 113, 5, 'Associated Buisnesses', 3, 1, 'R' );
                            $this->Ln(1);
                            $this->SetDrawColor(  0, 9, 67 );
