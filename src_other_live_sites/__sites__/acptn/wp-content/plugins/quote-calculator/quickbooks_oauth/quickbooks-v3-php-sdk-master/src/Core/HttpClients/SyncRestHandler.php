    ...
    */
    ...
    private function OAuth2APICall($baseURL, $queryParameters, $HttpMethod, $requestUri, $requestParameters, $requestBody, $throwExceptionOnError){
        ...
        if (CoreConstants::IntuitServicesTypeQBO ==$this->context->serviceType) {
			...
			//echo '<pre>'; var_dump( $requestUri, $HttpMethod, $httpHeaders,  $requestBody ); die();
        ...
        //for 3xx without direct, it will throw a 503 code and error saying: Invalid protected resource url, unable to generate signature base string
        if($faultHandler) {
			//echo '<pre>'; var_dump( $requestUri, $HttpMethod, $httpHeaders,  $requestBody, $faultHandler->getResponseBody() );
			error_log( print_r( $requestUri, true ) . PHP_EOL, 3, dirname( __FILE__ ) . '/qc_error.log' );
			error_log( print_r( $HttpMethod, true ) . PHP_EOL, 3, dirname( __FILE__ ) . '/qc_error.log' );
			error_log( print_r( $httpHeaders, true ) . PHP_EOL, 3, dirname( __FILE__ ) . '/qc_error.log' );
			error_log( print_r( $requestBody, true ) . PHP_EOL, 3, dirname( __FILE__ ) . '/qc_error.log' );
			error_log( print_r( $intuitResponse->getBody(), true ) . PHP_EOL, 3, dirname( __FILE__ ) . '/qc_error.log' );
			error_log( print_r( $intuitResponse->getHeaders(), true ) . PHP_EOL, 3, dirname( __FILE__ ) . '/qc_error.log' );
			error_log( '//////////////////////////////' . PHP_EOL, 3, dirname( __FILE__ ) . '/qc_error.log' );
            if($throwExceptionOnError == true){
