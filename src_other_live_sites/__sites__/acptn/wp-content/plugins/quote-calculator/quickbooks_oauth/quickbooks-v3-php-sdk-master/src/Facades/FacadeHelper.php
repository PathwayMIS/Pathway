...
class FacadeHelper{
   ...
   public static function reflectArrayToObject($classNameOrKeyName, $data, $throwException = TRUE) {
     if(!isset($classNameOrKeyName)){ echo ("The Class Name or Key Name cannot be NULL when generating Objects.");}
     if(!isset($data) || empty($data)){ echo ("The passed data cannot be NULL.");}
     if(is_object($data)){
       if(!Facadehelper::checkIfTheObjectIsAnInstanceOfTheClass($classNameOrKeyName,$data)){
         echo ("The assigned object is not an instance of required object:{" . $classNameOrKeyName . "}.");
       }else{
         return $data;
       }
     }
     ...
     if(FacadeHelper::isKeyInComplexList($classNameOrKeyName)){
         ...
         if(class_exists($clazz)){
                       ...
                       FacadeHelper::assignValue($currentObj, $key, $obj);
                         ...
                         foreach($val as $valobj){
                            if(!Facadehelper::checkIfTheObjectIsAnInstanceOfTheClass($key,$valobj)){
                              echo ("The assigned object is not an instance of required object:{" . $key . "}.");
                            }
                         }
                        ...
                        foreach ($val as $index => $element) {
                            if(is_object($element)){
                              ...
                              echo ("Format Error. Expect an element of an array or an object.");
                            }
                          }
            ...
            if($throwException)
            {
              echo ("The name value:{" . $classNameOrKeyName . "} can't be used to construct an Intuit Entity Object. Please check the name and try again.");
            }else{
              return NULL;
            }
         }
   ...
   private static function setKeyInComplexList($objectMethodName, $targetObject, $key, $val){
     ...
     if($property instanceof \ReflectionProperty){
       ...
       echo ("No Reflection Property Found.");
     }
   ...
   private static function getComplexListObject($objectMethodName, $key, $val){
     ...
     if(!isset($correspondingClassMethodOfFacadeHelper)) echo ("The given Method " . $objectMethodName . " can't find at FacadeHelper.php class");
   ...
   public static function getEnumType($clazz, $val){
      if(!isset($val)) echo ("Passed param for Enum can't be null.");
      if(class_exists($clazz)){
          if(is_object($val)){
              if($val instanceof $clazz){
                return $val;
              }else{
                echo ("The assigned obj to the enum class Type:{" . $clazz . "} is not matched.");
              }
          }
          $enumObj = new $clazz();
          //If $val is string
          if(is_array($val) && !empty($val)){
             ...
             echo ("Internal Error. The Type of val:" . get_class($val) . " is not handled.");
          }
          $enumObj->value = $firstElementValue;
          return $enumObj;
      }else{
        echo ("The Enum Type is not found");
      }
   }
   ...
   public static function getIPPReferenceTypeBasedOnArray($data){
      ...
      if(is_object($trimedDataArray)){
           if($trimedDataArray instanceof IPPReferenceType){
               ...
               echo ("The assigned obj to IPPReferenceType is not matched with IPPReferenceType.");
           }
      }
      //THe ReferenceDataType should only contain at most Two elements
      if(is_array($trimedDataArray)){
        if(sizeof($trimedDataArray) >= 3){
          echo ("Trying to construct IPPReferenceType based on Array. The array should contain at most two fields. name and value");
        }
        ...
        if(isset($trimedDataArray['value']) || isset($trimedDataArray['Value']) ) {
            ...
            echo ("Passed array has no key for 'Value' when contructing an ReferenceType");
        }
        ...
        echo ("Can't convert Passed Parameter to IPPReferenceType.");
      }
  }
  ...
  public static function getIPPId($data){
    ...
    if(!isset($data)) echo ("Passed param for IPPid can't be null");
    if(is_object($data)){
         if($data instanceof IPPid){
             return $data;
         }else{
             echo ("The assigned obj to IPPid is not matched with IPPid.");
         }
    }
    if(is_array($data)){
      ...
      echo ("Passed param for Ippid has either be an array or numeric value.");
    }
  ...
  public static function mergeObj($objA, $objB){
      if(get_class($objA) != get_class($objB)) echo ("Can't assign object value to a different type.");
  ...
  public static function cloneObj($obj){
      ...
      if(class_exists($clazz)){
         ...
         echo ("The passed object:{" . $clazz . "} does not exist.");
      }
  }
