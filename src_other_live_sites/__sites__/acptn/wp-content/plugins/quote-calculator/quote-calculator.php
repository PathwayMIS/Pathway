...
require_once( plugin_dir_path( __FILE__ ) . 'interract_with_xero/qc-xero.php' );
...
if ( ! function_exists ( 'quote_calculator_init' ) ) {
	function quote_calculator_init() {
		global $wpdb, $quote_calculator_plugin_info, $quote_calculator_options, $quote_calculator_baseUrl;
		if( ! session_id() ){
			session_start();
		}
		...
		if ( ! is_admin() || isset( $_GET['page'] ) && ( $_GET['page'] == 'quote_calculator' || $_GET['page'] == 'quote_calculator_margins_vat' || $_GET['page'] == 'quote_calculator_sync' || $_GET['page'] == 'quote_calculator_oauth' || $_GET['page'] == 'quote_calculator_editor' || $_GET['page'] == 'quote_calculator_xero' ) ) {
	...
	//echo '<pre>'; var_dump($_POST); die();
			...
			if( isset( $_POST['quote_calculator_category'] ) && ! empty( $_POST['quote_calculator_category'] ) && ! empty( $current_estimate_id ) ){
				...
				foreach( $_POST['quote_calculator_category'] as $category => $estimate_section_array ){
					foreach( $estimate_section_array as $estimate_section ){
						...
						//var_dump($wpdb->last_error, $wpdb->last_query);
					}
				}
			}
			// die();
		...
		if ( ! is_admin() && isset( $quote_calculator_options['xero_invoice_sent'] ) ) {
		    ?>
            <p>Sent.</p>
            <?php
            unset( $quote_calculator_options['xero_invoice_sent'] );
        }
		...
		if( ! is_admin() && isset( $_GET['_wp_nonce'] ) && isset( $_GET['quote_calculator_create_xero_invoice'] ) && wp_verify_nonce( $_GET['_wp_nonce'], 'quote_calculator_create_xero_invoice_action' ) ) {
			$qc_xero = new QC_Xero();
			if ( 'Connection error' !== $qc_xero->xero ) {
				$qc_xero->qc_create_invoice_in_xero();
			}
		}
...
if ( ! function_exists( 'quote_calculator_admin_menu' ) ) {
	function quote_calculator_admin_menu() {
		...
		/* Adding 'Sync with Quickbooks' page if Xero API isn't used */
		...
		/* Adding 'Sync with Xero' page if Quickbooks API isn't used */
		if ( empty( $quote_calculator_options['client_id'] ) && empty( $quote_calculator_options['client_secret'] ) ) {
			add_submenu_page( 'quote_calculator', __( 'Sync with Xero', 'quote_calculator' ), __( 'Sync with Xero', 'quote_calculator' ), 'manage_options', 'quote_calculator_xero', 'quote_calculator_xero' );
		}
	...
	function quote_calculator_sync(){
		...
		if( isset( $_POST['quote_calculator_admin_page'] ) && wp_verify_nonce( $_POST['quote_calculator_admin_page'], 'quote_calculator_admin_page' ) ){
			...
			if ( isset( $_POST['reset'] ) ) {
				$_POST['client_id'] = $_POST['client_secret'] = $quote_calculator_options['client_id'] = $quote_calculator_options['client_secret'] = '';
				$update_flag = true;
            }
		...
		<div class="wrap">
			...
			<form method="post" action="<?php echo admin_url( 'admin.php?page=quote_calculator_sync' ); ?>" novalidate="novalidate">
				...
				<p class="submit"><input name="submit" id="submit" class="button button-primary" value="<?php _e( 'Save Changes' ); ?>" type="submit"></p>
                <p class="submit"><input name="reset" id="reset-data" class="button button-primary" value="<?php _e( 'Reset Data' ); ?>" type="submit"></p>
				<?php if( ! isset( $_SESSION['code'] ) && ! empty( $quote_calculator_options['client_id'] ) && ! empty( $quote_calculator_options['client_secret'] ) ){
					$dataService = DataService::Configure(array(
						'auth_mode' => 'oauth2',
						'ClientID' => $quote_calculator_options['client_id'],
...
/*
* Function to display Xero page.
*/
if ( ! function_exists( 'quote_calculator_xero' ) ) {
	function quote_calculator_xero() {
		global $title, $quote_calculator_options;

		$connect_to_xero_img = plugin_dir_url( __FILE__ ) . 'images/xero-connect-blue.svg';

		/* Save xero consumer key and xero consumer secret to the database */
		if ( isset( $_POST['quote_calculator_xero'] ) && wp_verify_nonce( $_POST['quote_calculator_xero'], 'quote_calculator_xero' ) ) {

			if ( isset( $_POST['xero_consumer_key'] ) ) {
				$quote_calculator_options['xero_consumer_key'] = $_POST['xero_consumer_key'];
			}

			if ( isset( $_POST['xero_consumer_secret'] ) ) {
				$quote_calculator_options['xero_consumer_secret'] = $_POST['xero_consumer_secret'];
			}

			if ( isset( $_POST['xero_app_type'] ) ) {
				$quote_calculator_options['xero_app_type'] = $_POST['xero_app_type'];
			}

			if ( isset( $_POST['reset'] ) ) {
				$_POST['xero_consumer_key'] = $_POST['xero_consumer_secret'] = $quote_calculator_options['xero_consumer_key'] = $quote_calculator_options['xero_consumer_secret'] = '';
				$update_flag = true;
            }
		...
		/* $xero_app_type = isset( $quote_calculator_options['xero_app_type'] ) ? $quote_calculator_options['xero_app_type'] : ''; */

		?>
        <div class="wrap">
            <h1><?php echo $title; ?></h1>
			<h3 class="qc-xero-notice" >You can connect to Xero, using only Private app</h3>
            <form method="post" action="<?php echo admin_url( 'admin.php?page=quote_calculator_xero' ); ?>">
                <input name="action" value="update" type="hidden">
				<?php wp_nonce_field( 'quote_calculator_xero', 'quote_calculator_xero' ); ?>
                <!-- <div class="qc-xero-app-type-container">
                    <h3>Type of the Xero App</h3>
                    <label>Public App<input type="radio" name="xero_app_type" value="public" required <?php echo 'public' == $xero_app_type ? 'checked' : ''; ?>></label>
                    <label>Private App<input type="radio" name="xero_app_type" value="private" required <?php echo 'private' == $xero_app_type ? 'checked' : ''; ?>></label>
                </div> -->
                <table class="form-table qc-custom-table">
                    <thead>
                    <tr>
                        <th scope="row"><label for="client_id"><?php _e( 'Xero Consumer Key', 'quote_calculator' ); ?></label>
                        </th>
                        <th scope="row"><label for="client_secret"><?php _e( 'Xero Consumer Secret', 'quote_calculator' ); ?></label>
                        </th>
                    </tr>
                    </thead>
                    <tbody>
                    <tr>
                        <td><input name="xero_consumer_key" id="client_id" value="<?php echo $quote_calculator_options['xero_consumer_key']; ?>" class="regular-text" type="text" required></td>
                        <td><input name="xero_consumer_secret" id="xero_consumer_secret" value="<?php echo $quote_calculator_options['xero_consumer_secret']; ?>" class="regular-text" type="text" required></td>
                    </tr>
                    </tbody>
                </table>
                <p class="submit"><input name="submit" id="submit" class="button button-primary" value="<?php _e( 'Save Changes' ); ?>" type="submit"></p>
		...
		<?php require_once( plugin_dir_path( __FILE__ ) . 'interract_with_xero/qc-xero.php' );
		...
		if ( 'Connection error' !== $qc_xero->xero ) {
            ...
            /* if ( 'public' == $quote_calculator_options['xero_app_type'] && ! $_SESSION['xero_oauth']['xero_expires'] ) {
                ...
                if ( ! isset( $qc_xero->oauth_response ) ) {
                ...
                printf(
                    '<a href="%s"><img src="' . $connect_to_xero_img . '" alt="Connect to Xero"></a>',
                    $qc_xero->xero->getAuthorizeURL( $qc_xero->oauth_response['oauth_token'] )
                );

            } else { */
                $last_import_from = isset( $quote_calculator_options['xero_import_from'] ) ? ' from ' . $quote_calculator_options['xero_import_from'] : ''; ?>
                <p class="connected-message"><?php _e( 'Now you connected to Xero', 'quote_calculator' ); ?></p>

                <p><input id="submit-xero-import" class="button button-primary <?php if ( isset( $quote_calculator_options['xero_start_position'] ) ) echo 'hidden'; ?>" value="<?php _e( 'Import Xero Customers', 'quote_calculator' ); ?>" type="button"></p>

                <p class="import-clients-message <?php if ( ! isset( $quote_calculator_options['xero_start_position'] ) ) echo 'hidden'; ?>"><?php _e( 'Importing Customers', 'quote_calculator' ); if( ! empty( $quote_calculator_options['xero_start_position'] ) ) echo ' (' . __( 'from', 'quote_calculator' ) . ' ' . $quote_calculator_options['xero_start_position'] . ')'; ?> ... <img src="<?php echo admin_url( 'images/loading.gif' ); ?>" /></p>
                <p><?php _e( 'Last clients import at', 'quote_calculator' ); ?> <?php echo isset( $quote_calculator_options['xero_last_import'] ) ? date( 'd F Y', $quote_calculator_options['xero_last_import'] ) . $last_import_from : 'never'; ?></p>
            <?php /* } */
		}
...
* Function to set up ajax action.
*/
if ( ! function_exists ( 'qc_import_xero_customers' ) ) {
	function qc_import_xero_customers() {
		wp_clear_scheduled_hook( 'qc_xero_import_customers_hook' );
        QC_Xero::qc_xero_import_customers_function();
        wp_schedule_event( time(), '5minutes', 'qc_xero_import_customers_hook' );
...
if ( ! function_exists( 'quote_calculator_frontend_head' ) ) {
	function quote_calculator_frontend_head() {
		...
		wp_enqueue_style( 'quote_calculator_custom_style', plugins_url( 'css/custom-styles.css', __FILE__ ) );
		wp_enqueue_script( 'quote_calculator_custom_script', plugins_url( 'js/custom-script.js?1', __FILE__ ) );
...
if ( ! function_exists( 'quote_calculator_get_item' ) ) {
	function quote_calculator_get_item( $item_category, $item_name ){
		...
		$estimate_item_info = $wpdb->get_row( 'SELECT `items_object_id` FROM ' . $table_name . ' WHERE `items_name` LIKE "'. $item_name .'%" AND `items_parent_ref` = ' . $parent_id , ARRAY_A );
...
if( ! function_exists( 'quote_calculator_create_estimate' ) ){
	function quote_calculator_create_estimate(){
		global $wpdb, $quote_calculator_options, $quote_calculator_estimate, $quote_calculator_estimate_section, $quote_calculator_estimate_all_sections, $quote_calculator_estimate_customer, $create_estimate_message, $create_estimate_error, $quote_calculator_estimate_all_sections_qty, $quote_calculator_baseUrl, $quote_calculator_estimate_all_sections_tax, $quote_calculator_estimate_all_sections_tax_code;
		...
		if ( ! $dataService ){
			...
			if( ! empty( $quote_calculator_estimate ) ){
				...
				foreach( $quote_calculator_estimate_sections as $section ){
					...
					if( ! empty( $quantities ) && is_array( $quantities ) ){
						foreach( $quantities as $key => $value ){
							...
							$quote_calculator_estimate_all_sections_tax_code[] = '2';
						}
					} else {
						$quote_calculator_estimate_section['estimate_section_single_cost'] = money_format('%.2n', $quote_calculator_estimate_section['estimate_section_cost_without_vat'] );
				...
				$delivery_price = $wpdb->get_var( 'SELECT `delivery_price` FROM `' . $wpdb->base_prefix . 'quote_calculator_delivery` WHERE delivery_title="' . $quote_calculator_estimate['estimates_delivery'] . '"' );
				if( 'Tracked Courier' == $quote_calculator_estimate['estimates_delivery'] ){
					...
					$item_find = ItemHelper::getItemByID( $dataService, $item['items_object_id'] );
					$dataService->throwExceptionOnError(true);
					$item_find->UnitPrice = $delivery_price;
					$quote_calculator_estimate_all_sections[] = $item_find;
...
if( ! function_exists( 'quote_calculator_create_pdf' ) ){
	function quote_calculator_create_pdf(){
		if( isset( $_GET['quote_calculator_create_pdf'] ) && is_numeric( $_GET['quote_calculator_create_pdf'] ) ){
			global $flag_header, $flag_footer, $status, $wpdb;
			...
			if( ! empty( $estimate ) ){
				...
				if ( ! empty( $estimate['estimates_delivery'] ) ) {
					$delivery_price = $wpdb->get_results( 'SELECT `delivery_price` FROM `' . $wpdb->base_prefix . 'quote_calculator_delivery` WHERE delivery_title="' . $estimate['estimates_delivery'] . '"', ARRAY_A );
					$delivery_price = $delivery_price[0];
					$delivery_price = number_format( $delivery_price["delivery_price"], 2 );
					$vat = $wpdb->get_results( 'SELECT `estimate_section_vat` FROM `' . $wpdb->base_prefix . 'quote_calculator_estimate_sections` WHERE estimate_sections_estimate_id="' . $estimate['estimates_id'] . '"', ARRAY_A );
					$vat = 0.01 * $vat[0]['estimate_section_vat'];
					$delivery_price_inc_vat = $delivery_price * $vat + $delivery_price;
				}
				...
				class PDF extends FPDF{
					...
					function Header()	{
						...
						$rgb = array( 158, 51, 147 );
						//changing logo from admin side
						/*if ( isset( $qc_dashboard_options['dashboard_details'] ) ) {
							$logo = $qc_dashboard_options['dashboard_details']['logo'];
							if ( ! empty( $logo ) ) {
								$logo = strstr( $logo, 'wp-content' );
							}
							if ( ! file_exists( $logo ) ) {
								$logo = 'https://acp.pathwaymis.co.uk/wp-content/uploads/sites/PathwayMIS_Logo_Colour-1.jpg';
							}
							if ( ! empty( $qc_dashboard_options['dashboard_details']['titles_color'] ) ) {
								$rgb = self::hex2rgb( $qc_dashboard_options['dashboard_details']['titles_color'] );
                            }
						} else {
							$logo = '';
                        }*/
						...
						if( false === $flag_header ){
							...
							// Logo
							/*if ( ! empty( $logo ) ) {
							    $this->Image( $logo, 10, 10, 45 );
							}*/
                            $this->Image( plugin_dir_path( __FILE__ ) . 'images/logo_tonbridge.png', 10, 10, 45 );
                            $this->Image( plugin_dir_path( __FILE__ ) . 'images/google_rate.jpg', 145, 10, 45 );
                            $this->Ln(30);
					...
					function Footer(){
						...
						$rgb = array( 158, 51, 147 );
						if ( isset( $qc_dashboard_options['dashboard_details'] ) && ! empty( $qc_dashboard_options['dashboard_details']['titles_color'] ) ) {
                            $rgb = self::hex2rgb( $qc_dashboard_options['dashboard_details']['titles_color'] );
						}
						$r = $rgb[0];
						$g = $rgb[1];
						$b = $rgb[2];
						if( true === $flag_footer ) {
                            $this->Image( plugin_dir_path( __FILE__ ) . 'images/offpaper_logo.png', 182, 270, 20 );
                            $this->Image( plugin_dir_path( __FILE__ ) . 'images/creative_gifts_logo.png', 152, 270, 20 );
                            $this->Image( plugin_dir_path( __FILE__ ) . 'images/logo_maidstone.png', 122, 270, 20 );
                            $this->SetY(-35);
                            $this->SetFont( 'Arial', '', 10 );
                            $this->SetTextColor( 128, 128, 128 );
                            $this->Cell( 0, 5, 'We Know Print Ltd T/A', 0, 1, 'L' );
                            $this->SetTextColor( $r, $g, $b );
                            $this->Cell( 0, 5, $qc_dashboard_options['company_details']['company_name'], 0, 1, 'L' );
                            ...
                            $this->Cell( 23, 5, $qc_dashboard_options['company_details']['phone'], 0, 0, 'L'  );
                            $this->SetTextColor( $r, $g, $b );
                            $this->Cell( 5, 5, urldecode( '%95' ), 0, 0, 'L'  );
                            $this->SetTextColor( 128, 128, 128 );
                            $this->Cell( 33, 5, $qc_dashboard_options['company_details']['email'], 0, 0, 'L'  );
                            $this->SetTextColor( $r, $g, $b );
                            $this->Cell( 5, 5, urldecode( '%95' ), 0, 0, 'L'   );
                            $this->SetTextColor( 128, 128, 128 );
                            $this->Cell( 20, 5, $qc_dashboard_options['company_details']['company_url'], 0, 1, 'L' );
                            $this->Ln(1);
                            $this->SetDrawColor( $r, $g, $b );
                            $this->Cell( 192, 0.8, '', 'B', 1, 'C' );
                            $this->Ln(2);
                            $this->SetFont( 'Arial', '', 7 );
                            $this->Cell( 0, 5, 'Registered Office: '. $qc_dashboard_options['company_details']['registered_address_line1'] . ', ' . ( ! empty( $qc_dashboard_options['company_details']['registered_address_line2'] ) ? $qc_dashboard_options['company_details']['registered_address_line2'] . ', ' : '' ) . ( ! empty( $qc_dashboard_options['company_details']['registered_address_line3'] ) ? $qc_dashboard_options['company_details']['registered_address_line3'] . ', ' : '' ) . $qc_dashboard_options['company_details']['registered_address_city'] . ', ' . $qc_dashboard_options['company_details']['registered_address_location'] . ' ' . $qc_dashboard_options['company_details']['registered_address_postal_code'] . '     Company Registration Number: ' . $qc_dashboard_options['company_details']['registration_number'] . '     Registered in England and Wales.   VAT No: ' . $qc_dashboard_options['company_details']['vat_number'] , 0, 1, 'L' );
						} else {
							$this->SetY(-15);
				...
				$pdf = new PDF();
				if ( isset( $qc_dashboard_options['dashboard_details'] ) && ! empty( $qc_dashboard_options['dashboard_details']['titles_color'] ) ) {
					$rgb = $pdf->hex2rgb( $qc_dashboard_options['dashboard_details']['titles_color'] );
				}
				$r = $rgb[0];
				$g = $rgb[1];
				$b = $rgb[2];
				...
				if ( isset( $delivery_price ) && 0 != $delivery_price && 'delivery_note' != $status ) {
					$pdf->SetFont( 'Arial', 'B', 10 );
					$pdf->Cell( 30, 6, 'Delivery', 0, 0 );

					$pdf->SetFont( 'Arial', '', 10 );
					$pdf->Cell( 30, 6, $estimate['estimates_delivery'], 0, 0 );

					$pdf->Cell( 40);
					$pdf->Cell( 20, 6, urldecode( '%A3' ) . $delivery_price, 0, 0 );

					if ( isset( $delivery_price_inc_vat ) ) {
						$pdf->SetFont( 'Arial', '', 8 );
						$pdf->Cell( 20, 6, '( ' . urldecode( '%A3' ) . $delivery_price_inc_vat . ' incl vat )', 0, 1);
					}
					$pdf->Ln( 4 );
				}
...
/* Xero */
add_action( 'wp_ajax_import_xero_customers', 'qc_import_xero_customers' );
add_action( 'qc_xero_import_customers_hook', array( 'QC_Xero', 'qc_xero_import_customers_function' ) );
/*qc_xero_import_customers_hook*/
/********/
